...
def write_rp(obs, mod_h, mod_q, result, out_dir):
    ...
    with open(path_params, 'w') as pf:
        pf.write('X;Y;b;Ks;S;ret;slope;rainfall;SofSq;SS_h;SS_q;NashSutcliffe_h;NashSutcliffe_q\n')
        for ix in result.x:
    ...
    with open(path_params, 'a') as pf:
        ...
        pf.write('{:1.3e};'.format(sum_of_squares(obs.data_h.val, mod_h.val)))
        pf.write('{:1.3e};'.format(sum_of_squares(obs.data_q.val, mod_q.val)))
